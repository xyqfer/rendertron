...
class DatastoreCache {
    ...
    async clearCache() {
        ...
        const entityKeys = entities.map((entity) => entity[datastore_1.Datastore.KEY]);
    ...
    async cacheContent(key, headers, payload) {
        ...
        if (parseInt(this.config.cacheConfig.cacheMaxEntries) !== -1) {
            ...
            this.datastore.runQuery(query, function (err, entities) {
                ...
                const dataStoreCache = (entities || []).map((entity) => entity[datastore_1.Datastore.KEY]);
    ...
    async getCachedContent(ctx, key) {
...
//# sourceMappingURL=datastore-cache.js.map